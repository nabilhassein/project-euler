...
array = [r0, r1, r2, r3, r4, r5, r6, r7, r8, r9, r10, r11, r12, r13, r14, r15, r16, r17, r18, r19]
...
def maxhorizontal(A, count):
  maxproduct = float("-inf")
  for row in A:
    for i in range(0, len(row) - count + 1):
      slice = row[i : i+count]
      maxproduct = max(maxproduct, product(slice))
  return maxproduct

def maxvertical(A, count):
  return maxhorizontal(zip(*A), count)

def maxdiagonalright(A, count):
  maxproduct = float("-inf")
  temp = [0] * count
  for i in range(0, len(A) - count + 1):
    for j in range(0, len(A) - count + 1):
      for k in range(0, count):
        temp[k] = A[i+k][j+k]
      maxproduct = max(maxproduct, product(temp))
  return maxproduct

def maxdiagonalleft(A, count):
  maxproduct = float("-inf")
  temp = [0] * count
...
def problem11(A, count):
  return max(maxhorizontal(A, count), maxvertical(A, count),
             maxdiagonalright(A, count), maxdiagonalleft(A, count))

print problem11(array, 4)